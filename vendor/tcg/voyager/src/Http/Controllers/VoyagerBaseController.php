...
public function americ_api ($em_type,$em_data,$method,$id=""){
    ...
    public function update(Request $request, $id)
    {
        ...
        if($dataType->name=="product_categories")
        {
            $ac_id=ProductCategory::find($id)->ac_id;
            $dad=0;
            if(isset($data->parent_id))
            {
                $dad=ProductCategory::find($data->parent_id)->ac_id;
            }
            ...
            $em_data="
            {
             ...
             $this->americ_api($em_type,$em_data,"PUT",$ac_id);
    
        }
        else if($dataType->name =="products")
        {
            $ac_id=Product::find($id)->ac_id;
            $em_type="products";
            $id_item=1;
            $product=1;
            if(isset($product))
            $id_item=$product+1;
            $ac_category=ProductCategory::find($data->product_category_id)->ac_id;
            $body=strip_tags($data->body);
          
            $hidden= $data->is_active ? 0:1;

            $em_data="
            {
                'sort_order'          :  {$id_item},
                'item_number'         :  '$data->code',
                'primary_category_id' :   $ac_category,
                'item_name'           : '{$body}',
                'weight'              :  {$data->size},
                'price'               :  {$data->price},
                'is_hidden'           :   $hidden,
        ...
        $ac_id= $this->americ_api($em_type,$em_data,"PUT",$ac_id);
        $ac_img_id=Product::find($id)->ac_img_id;
    if(isset($data->image)){
     
        $im=str_replace("\\",'/',$data->image);
        $image="https://wenfee.com/strorge/".$im;

        $em_type="product_pictures.json";
        ...
        ";
         $this->americ_api($em_type,$em_data,"POST");
    }
    ...
    public function store(Request $request)
    {
        ...
        if($dataType->name=="product_categories")
        {
            ...
            $ac_id =$this->americ_api($em_type,$em_data,"POST");

            $delete_id =$this->americ_api($em_type,$em_data,"POST");
            $this->americ_api($em_type,$em_data,"DELETE",$delete_id);
      
            $id= ProductCategory::orderBy('id','DESC')->get()->first()->id;
            ProductCategory::find($id)->update(["ac_id" => $ac_id]);
        }
        ...
        else if($dataType->name =="products")
            {
                ...
                $image="https://wenfee.com/jasmine/thenewwenfee/storage/app/public/products/".$monthName.date('Y').'/'.$im;
                $hidden= $data->is_active ? 0:1;
            ...
            ";
            $ac_img_id= $this->americ_api($em_type,$em_data,"POST");
             $id= Product::orderBy('id','DESC')->get()->first()->id;
                  Product::find($id)->update(["ac_id" => $ac_id,
                                               "ac_img_id"=>$ac_img_id
                  ]);
            }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        {   
            ...
            $em_data="{}";
             $this->americ_api($em_type,$em_data,"DELETE",$ac_id);
